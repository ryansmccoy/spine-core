...
makes this chain explicit, trackable, and restartable.

ARCHITECTURE
────────────
    ┌──────────┐   params   ┌──────────┐  result   ┌─────────┐
    │ Validate │─────────▶│  Fetch   │─────────▶│ Analyse │
    └──────────┘          └──────────┘          └────┬────┘
                                                    │
                                               result
                                                    ▼
    ...
    Each arrow carries a dict of results from the previous step.
    Steps are registered as async handler functions and dispatched
    by EventDispatcher through a MemoryExecutor.

KEY COMPONENTS
──────────────
    Component         Role
    ───────────────── ─────────────────────────────────
    HandlerRegistry   Maps (namespace, name) → async fn
    EventDispatcher   submit_task → executor → RunRecord
    MemoryExecutor    Runs handlers in-process
    WorkSpec          Describes a unit of work

BEST PRACTICES
──────────────
• Keep step handlers pure — input dict in, output dict out.
• Name steps for the *action* (validate_input, not step_1).
• Return enough context for downstream steps to be self-contained.
• Use TrackedWorkflowRunner (05) for production persistence.
