...
See Also:
    - ``01_workflow_basics.py``    — simpler sequential version
    - ``07_parallel_dag.py``       — pure DAG timing demo
    - ``examples/07_real_world/05_sec_filing_workflow.py`` — async version
    - py-sec-edgar project         — ``SEC`` client, ``SectionExtractor``

Run:
    python examples/04_orchestration/17_sec_etl_workflow.py

    # Optional — real SEC EDGAR mode:
    SEC_USER_AGENT="MyApp admin@example.com" python examples/04_orchestration/17_sec_etl_workflow.py
