#!/usr/bin/env python3
"""Pipeline vs Workflow — Understanding the differences.

WHY THIS DISTINCTION MATTERS
────────────────────────────
Choosing the wrong abstraction leads to either:
• Monolithic "god pipelines" that can't be reused, or
• Over-orchestrated workflows for simple transformations.
Spine separates the two so that pipelines stay composable and
...
──────────────
    Question                     Answer         Use
    ──────────────────────────── ────────────── ────────────
    Does it transform data?      Yes            Pipeline
    Does it have branching?      Yes            Workflow
    Is it reusable across        Yes            Pipeline
      different contexts?
    Does it coordinate           Yes            Workflow
      multiple pipelines?

    → Workflows INVOKE Pipelines (they work together!)

ARCHITECTURE
────────────
    Pipeline (stateless transform):
    ┌─────────┐   ┌───────────┐   ┌──────┐
...
Run: python examples/04_orchestration/02_pipeline_vs_workflow.py

See Also:
    01_workflow_basics — basic sequential workflow
    05_choice_branching — conditional workflow logic
    docs/guides/pipeline_vs_workflow.md — detailed decision guide
"""
import asyncio
...
# === Pipeline-style handlers (transform data) ===

async def pipeline_extract(params: dict) -> dict:
    """Extract raw data."""
...
async def pipeline_transform(params: dict) -> dict:
    """Transform raw data."""
...
async def pipeline_load(params: dict) -> dict:
...
async def main():
    ...
    # Pipeline handlers
    registry.register("task", "extract", pipeline_extract)
    registry.register("task", "transform", pipeline_transform)
    registry.register("task", "load", pipeline_load)
    ...
    handlers = {
        "task:extract": pipeline_extract,
        "task:transform": pipeline_transform,
        "task:load": pipeline_load,
    ...
    print("\n" + "=" * 60)
    print("[OK] Pipeline vs Workflow Complete!")
    print("=" * 60)
