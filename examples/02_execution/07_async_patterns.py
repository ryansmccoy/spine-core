...
Data pipelines often need to coordinate multiple concurrent operations::
    ...
    # Concurrent (fast) — 5 API calls in parallel = ~2 seconds
    results = await asyncio.gather(
        *[fetch_data(t) for t in ["AAPL", "MSFT", "GOOG", "AMZN", "META"]]
    )

But naive concurrency causes problems:
    - **Unbounded parallelism** → API rate limits hit
    - **No error isolation** → One failure cancels everything
    - **No progress tracking** → Can't report completion percentage

Spine-core's async patterns solve these with structured concurrency.
...
::
    ...
    Best Practice: Prefer ``asyncio.TaskGroup`` (Python 3.11+) for
    structured concurrency over raw ``asyncio.gather()``.
...
Run this example:
    python examples/02_execution/07_async_patterns.py

See Also:
    - :mod:`spine.execution.executors` — MemoryExecutor (async execution)
    - ``examples/02_execution/15_async_local_executor.py`` — AsyncLocalExecutor
...
def setup_dispatcher():
    ...
    return EventDispatcher(executor=executor, registry=registry)
