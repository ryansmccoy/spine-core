#!/usr/bin/env python3
"""Operation Data Processing — Manifest, rejects, and work items.

WHY OPS-LAYER DATA TRACKING
────────────────────────────
Operations process data.  The ops layer tracks *what* was processed
...
Run: python examples/10_operations/07_operation_data.py
...
def _seed_work_items(conn) -> None:
    ...
    for item_id, domain, workflow, pk, state, worker, ts in items:
        conn.execute(
            "INSERT INTO core_work_items "
            "(id, domain, workflow, partition_key, desired_at, state, locked_by, "
            "params_json, created_at, updated_at) "
            "VALUES (?, ?, ?, ?, ?, ?, ?, NULL, ?, ?)",
            (item_id, domain, workflow, pk, ts, state, worker, ts, ts),
        )
    conn.commit()
...
def main():
    print("=" * 60)
    print("Operations Layer — Operation Data Processing")
    print("=" * 60)
