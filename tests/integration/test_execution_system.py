...
class TestExecutionLedger:
    ...
    def test_create_and_get_execution(self, conn):
        ...
        assert retrieved.workflow == "finra.otc.ingest"
    ...
    def test_update_status_lifecycle(self, conn):
        ...
        execution = Execution.create(workflow="test.workflow")
    ...
    def test_list_executions_with_filters(self, conn):
        ...
        for i in range(5):
            exec1 = Execution.create(workflow="workflow.a")
            ledger.create_execution(exec1)
            
            exec2 = Execution.create(workflow="workflow.b")
        ...
        # Filter by workflow
        workflow_a = ledger.list_executions(workflow="workflow.a")
        assert len(workflow_a) == 5
        assert all(e.workflow == "workflow.a" for e in workflow_a)
...
class TestDLQManager:
    ...
    def test_add_and_get(self, conn):
        ...
        assert entry.workflow == "finra.otc.ingest"
    ...
    def test_list_unresolved(self, conn):
        ...
        # Add some entries
        dlq.add_to_dlq("exec-1", "workflow.a", {}, "Error 1")
        dlq.add_to_dlq("exec-2", "workflow.a", {}, "Error 2")
        entry3 = dlq.add_to_dlq("exec-3", "workflow.b", {}, "Error 3")
        ...
        # Filter by workflow
        unresolved_a = dlq.list_unresolved(workflow="workflow.a")
    ...
    def test_resolve(self, conn):
        ...
        entry = dlq.add_to_dlq("exec-123", "test.workflow", {}, "Error")
    ...
    def test_count_unresolved(self, conn):
        ...
        dlq.add_to_dlq("exec-1", "workflow.a", {}, "Error 1")
        dlq.add_to_dlq("exec-2", "workflow.a", {}, "Error 2")
        dlq.add_to_dlq("exec-3", "workflow.b", {}, "Error 3")
        
        assert dlq.count_unresolved() == 3
        assert dlq.count_unresolved(workflow="workflow.a") == 2
...
class TestExecutionRepository:
    ...
    def test_get_execution_stats(self, conn):
        ...
        assert stats["workflow_counts"]["workflow.a"] == 5
        assert stats["workflow_counts"]["workflow.b"] == 3
    ...
    def test_get_recent_failures(self, conn):
        ...
        for i in range(5):
            execution = Execution.create(workflow="failing.workflow")
            ledger.create_execution(execution)
            ledger.update_status(execution.id, ExecutionStatus.RUNNING)
            ledger.update_status(
                execution.id,
        ...
        assert all(f["workflow"] == "failing.workflow" for f in failures)

    def test_get_workflow_throughput(self, conn):
        """Test getting workflow-specific throughput metrics."""
        ledger = ExecutionLedger(conn)
        repo = ExecutionRepository(conn)
        
        # Create executions for one workflow
        for i in range(10):
            execution = Execution.create(workflow="target.workflow")
            ledger.create_execution(execution)
            ledger.update_status(execution.id, ExecutionStatus.RUNNING)
            if i < 8:
        ...
        throughput = repo.get_workflow_throughput("target.workflow", hours=24)
    ...
    def test_get_queue_depth(self, conn):
        ...
        for i in range(3):
            execution = Execution.create(workflow="test.workflow", lane="fast")
            ledger.create_execution(execution)
        
        for i in range(5):
            execution = Execution.create(workflow="test.workflow", lane="slow")
            ledger.create_execution(execution)
        
        # Start one to remove from queue
        started = Execution.create(workflow="test.workflow", lane="fast")
...
class TestEndToEndScenario:
    ...
    def test_full_execution_lifecycle_with_failure_and_dlq(self, conn):
        ...
        workflow = "finra.otc.ingest"
        params = {"week_ending": "2025-01-09", "tier": "NMS_TIER_1"}
        lock_key = f"{workflow}:2025-01-09:NMS_TIER_1"
