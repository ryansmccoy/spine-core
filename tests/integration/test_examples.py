...
Uses :class:`ExampleRegistry` for auto-discovery — new example
directories are picked up automatically with no list to maintain.

Run with:
    pytest tests/integration/test_examples.py -v -m integration
    pytest tests/integration/test_examples.py -v -k "core" -m integration
"""
...
# Resolve examples directory and build registry
EXAMPLES_DIR = Path(__file__).parent.parent.parent / "examples"

# Import the registry from the examples package
sys.path.insert(0, str(EXAMPLES_DIR))
from _registry import ExampleRegistry  # noqa: E402

_registry = ExampleRegistry(EXAMPLES_DIR)


def get_all_subdir_examples() -> list[tuple[str, Path]]:
    """Auto-discover all examples via the registry."""
...
class TestSubdirExamples:
    """Test examples from all subdirectories."""
    
    @pytest.mark.parametrize("name,path", get_all_subdir_examples())
    def test_example_runs(self, name, path):
    ...
    @pytest.mark.parametrize("name,path", get_all_subdir_examples())
    def test_example_has_docstring(self, name, path):
