...
def make_execution(pipeline: str, params: dict = None) -> Execution:
    """Helper to create an Execution object."""
    return Execution.create(pipeline=pipeline, params=params or {})
...
class TestGetPipelineThroughput:
    ...
    def test_calculates_throughput(self, ledger, repo):
        ...
        assert stats["pipeline_counts"].get("throughput.test", 0) == 5
    ...
    def test_includes_failure_rate(self, ledger, repo):
        ...
        assert stats["failure_rate_by_pipeline"].get("mixed.pipeline", 0) == 50.0
