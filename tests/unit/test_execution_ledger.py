...
class TestCreateExecution:
    ...
    def test_create_stores_all_fields(self, ledger, conn):
        ...
        assert row[1] == "test.pipeline"  # pipeline
        assert "key" in row[2]  # params (JSON)
...
class TestGetExecution:
    ...
    def test_get_existing(self, ledger):
        ...
        assert retrieved.pipeline == execution.pipeline
...
class TestListExecutions:
    ...
    def test_list_with_limit(self, ledger):
        ...
        for i in range(10):
            ledger.create_execution(Execution.create(pipeline="test.pipeline"))
        
        results = ledger.list_executions(limit=5)
        assert len(results) == 5

    def test_list_filter_by_pipeline(self, ledger):
        """Filter by pipeline name."""
        ledger.create_execution(Execution.create(pipeline="pipeline.a"))
        ledger.create_execution(Execution.create(pipeline="pipeline.a"))
        ledger.create_execution(Execution.create(pipeline="pipeline.b"))
        
        results = ledger.list_executions(pipeline="pipeline.a")
        
        assert len(results) == 2
        assert all(r.pipeline == "pipeline.a" for r in results)

    def test_list_filter_by_status(self, ledger):
        """Filter by status."""
        exec1 = Execution.create(pipeline="test")
        ledger.create_execution(exec1)
        ledger.update_status(exec1.id, ExecutionStatus.COMPLETED)
        
        exec2 = Execution.create(pipeline="test")
