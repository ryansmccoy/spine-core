...
def run_cli(args: list[str], check: bool = True, env: dict[str, str] | None = None) -> subprocess.CompletedProcess:
    """
...
def test_scenario_basic_cross_domain(db_path: Path) -> bool:
    ...
    try:
        ...
        result = run_cli([
            "run", "run", "reference.exchange_calendar.ingest_year",
            "--year", "2025",
            "--exchange-code", "XNYS",
            "--file", str(CALENDAR_2025_FILE),
        ], check=False)
        ...
        result = run_cli([
            "run", "run", "finra.otc_transparency.ingest_week",
            "--week-ending", WEEK_2025,
            "--tier", TEST_TIER,
            "--file", str(OTC_WEEK_2025),
        ], check=False)
        ...
        result = run_cli([
            "run", "run", "finra.otc_transparency.aggregate_week",
            "--week-ending", WEEK_2025,
            "--tier", TEST_TIER,
        ], check=False)
        ...
        result = run_cli([
            "run", "run", "finra.otc_transparency.compute_volume_per_day",
            "--week-ending", WEEK_2025,
            "--tier", TEST_TIER,
            "--exchange-code", "XNYS",
        ], check=False)
...
def test_scenario_year_boundary(db_path: Path) -> bool:
    ...
    try:
        ...
        result = run_cli([
            "run", "run", "finra.otc_transparency.ingest_week",
            "--week-ending", WEEK_YEAR_BOUNDARY,
            "--tier", TEST_TIER,
            "--file", str(OTC_WEEK_YEAR_BOUNDARY),
        ], check=False)
...
def test_scenario_asof_dependency(db_path: Path) -> bool:
    ...
    try:
        ...
        run_cli([
            "run", "run", "finra.otc_transparency.aggregate_symbols",
            "--week-ending", WEEK_2025,
            "--tier", TEST_TIER,
        ], check=False)
        
        # Step 3: Run with explicit calendar_capture_id
        print(f"  → Running volume per day with pinned capture_id...")
        result = run_cli([
            "run", "run", "finra.otc_transparency.compute_volume_per_day",
            "--week-ending", WEEK_2025,
            "--tier", TEST_TIER,
            "--exchange-code", "XNYS",
            "--calendar-capture-id", capture_id_v1,
        ], check=False)
...
def main() -> int:
    ...
    with temp_database() as db_path:
        ...
        # Run test scenarios
        results.append(("Scenario 1: Basic Cross-Domain", test_scenario_basic_cross_domain(db_path)))
