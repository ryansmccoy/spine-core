...
class TestRealFINRATradingAnalytics:
    ...
    def test_end_to_end_real_analytics(self, temp_db):
        ...
        # Step 2: Normalize
        capture_id = f"finra.otc_transparency:{tier}:{week_ending}:{captured_at[:10].replace('-', '')}"
        ...
        self._ingest_real_csv(conn, tier2_file, week_ending, "NMS_TIER_2", execution_id, batch_id, captured_at)
        self._ingest_real_csv(conn, otc_file, week_ending, "OTC", execution_id, batch_id, captured_at)
        
        # Normalize other tiers
        cap_tier2 = f"finra.otc_transparency:NMS_TIER_2:{week_ending}:{captured_at[:10].replace('-', '')}"
        cap_otc = f"finra.otc_transparency:OTC:{week_ending}:{captured_at[:10].replace('-', '')}"
        
        self._normalize_data(conn, week_ending, "NMS_TIER_2", execution_id, batch_id, captured_at, cap_tier2)
        self._normalize_data(conn, week_ending, "OTC", execution_id, batch_id, captured_at, cap_otc)
        
        # Compute venue volume for all tiers
        self._compute_venue_volume(conn, week_ending, "NMS_TIER_2", cap_tier2, execution_id, batch_id)
        self._compute_venue_volume(conn, week_ending, "OTC", cap_otc, execution_id, batch_id)
        
        tier_split_rows = self._compute_tier_split(
            conn, week_ending, execution_id, batch_id
        )
    ...
    def test_idempotency_and_asof(self, temp_db):
        ...
        self._ingest_real_csv(conn, test_file, week_ending, tier, exec_id_1, batch_id_1, captured_at_1)
        self._normalize_data(conn, week_ending, tier, exec_id_1, batch_id_1, captured_at_1, capture_id_1)
        venue_vol_1 = self._compute_venue_volume(conn, week_ending, tier, capture_id_1, exec_id_1, batch_id_1)
        ...
        venue_vol_2 = self._compute_venue_volume(conn, week_ending, tier, capture_id_1, exec_id_2, batch_id_2)
        ...
        self._normalize_data(conn, week_ending, tier, exec_id_3, batch_id_3, captured_at_3, capture_id_3)
        venue_vol_3 = self._compute_venue_volume(conn, week_ending, tier, capture_id_3, exec_id_3, batch_id_3)
    ...
    def test_venue_share_invariants(self, temp_db):
        ...
        self._ingest_real_csv(conn, test_file, week_ending, tier, execution_id, batch_id, captured_at)
        self._normalize_data(conn, week_ending, tier, execution_id, batch_id, captured_at, capture_id)
        ...
        for row in results:
            ...
            assert abs(total_share - 1.0) < 0.0001, f"{symbol}: shares sum to {total_share}, not 1.0"
    ...
    def _ingest_real_csv(self, conn, file_path, week_ending, tier, execution_id, batch_id, captured_at):
        ...
        with open(file_path, "r") as f:
            ...
            for row in reader:
                ...
                """, (
                    ...
                    mpid, total_shares, total_trades,
    ...
    def _normalize_data(self, conn, week_ending, tier, execution_id, batch_id, captured_at, capture_id):
        """Normalize raw data."""
        conn.execute("""
    ...
    def _compute_venue_volume(self, conn, week_ending, tier, capture_id, execution_id, batch_id):
        ...
        """, (week_ending, tier, capture_id)).fetchall()
        ...
        for vv in venue_volumes:
            ...
            """, (
                execution_id, batch_id, str(vv.week_ending), vv.tier, vv.symbol,
                vv.mpid, vv.venue_name, vv.total_volume, vv.trade_count,
                vv.calc_name, vv.calc_version, vv.captured_at, vv.capture_id,
    ...
    def _compute_venue_share(self, conn, week_ending, tier, capture_id, execution_id, batch_id):
        ...
        for vs in venue_shares:
            ...
            """, (
                execution_id, batch_id, str(vs.week_ending), vs.tier, vs.symbol,
                vs.mpid, vs.venue_name, vs.venue_volume, vs.total_symbol_volume,
                vs.venue_share, vs.calc_name, vs.calc_version,
                vs.captured_at, vs.capture_id, datetime.now(UTC).isoformat()
            ))
    ...
    def _compute_hhi(self, conn, week_ending, tier, capture_id, execution_id, batch_id):
        ...
        for hhi in hhi_rows:
            ...
            """, (
                execution_id, batch_id, str(hhi.week_ending), hhi.tier, hhi.symbol,
                hhi.hhi, hhi.venue_count, hhi.total_symbol_volume,
                hhi.calc_name, hhi.calc_version, hhi.captured_at, hhi.capture_id,
                datetime.now(UTC).isoformat()
            ))
    ...
    def _compute_tier_split(self, conn, week_ending, execution_id, batch_id):
        ...
        for ts in tier_splits:
            ...
            """, (
                execution_id, batch_id, str(ts.week_ending), ts.tier, ts.symbol,
                ts.tier_volume, ts.total_symbol_volume_all_tiers, ts.tier_volume_share,
                ts.calc_name, ts.calc_version, ts.captured_at, ts.capture_id,
                datetime.now(UTC).isoformat()
            ))
    ...
    def _assert_invariants(self, conn, week_ending, tier, capture_id):
        ...
        for row in shares_check:
            assert abs(row["total"] - 1.0) < 0.0001, f"Invariant violated: {row['symbol']} shares sum to {row['total']}"
        ...
        assert len(vs_bounds) == 0, f"Invariant violated: found {len(vs_bounds)} venue shares out of bounds"
