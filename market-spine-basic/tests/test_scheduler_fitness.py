...
class TestSchedulerFitness:
    ...
    def test_retry_on_failure_then_success(self, conn):
        ...
        # Verify PENDING
        state = conn.execute(
            "SELECT state, attempt_count FROM core_work_items WHERE id = ?",
            (work_id,)
        ).fetchone()
        ...
        state = conn.execute("SELECT state FROM core_work_items WHERE id = ?", (work_id,)).fetchone()
        ...
        assert state["attempt_count"] == 1  # Attempt count from failure only (success doesn't increment)
    ...
    def test_max_attempts_exhausted(self, conn):
        ...
        state = conn.execute(
            "SELECT state, attempt_count, last_error FROM core_work_items WHERE id = ?",
            (work_id,)
        ).fetchone()
    ...
    def test_failed_work_retry_command(self, conn):
        ...
        # Fail once
        self._claim_work(conn, work_id, worker_id="worker-1")
        self._record_failure(conn, work_id, error="Database lock timeout", next_attempt_delay_seconds=None)
    ...
    def _record_success(self, conn, work_id, execution_id, row_count, captured_at=None, capture_id=None):
        ...
        # Update manifest
        work = conn.execute(
            "SELECT domain, pipeline, partition_key FROM core_work_items WHERE id = ?",
            (work_id,)
        ).fetchone()
