...
class QualityRunner:
    ...
    def run_all(self, context: dict, partition_key: dict[str, Any] = None) -> dict[str, QualityStatus]:
        """
    ...
    def _record(self, check: QualityCheck, result: QualityResult, partition_key: dict[str, Any]) -> None:
        ...
        values = (
            ...
            result.status.value,
            result.message,
            json.dumps(result.actual_value, default=str) if result.actual_value is not None else None,
            json.dumps(result.expected_value, default=str) if result.expected_value is not None else None,
            self.execution_id,
            self.batch_id,
