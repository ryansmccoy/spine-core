...
def try_result[T](f: Callable[[], T]) -> Result[T]:
    """
...
def collect_results[T](results: list[Result[T]]) -> Result[list[T]]:
    """
...
def collect_all_errors[T](results: list[Result[T]]) -> Result[list[T]]:
    """
...
@overload
def from_optional[T](value: T, error: Exception) -> Ok[T]: ...
...
def from_optional[T](value: T | None, error: Exception) -> Result[T]:
    """
