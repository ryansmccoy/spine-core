"""FastAPI integration - unified /runs API.

This module provides a FastAPI router for the execution API. All work types
(tasks, pipelines, workflows, steps) are managed through a single /runs endpoint.

NO separate /tasks, /pipelines, /executions endpoints - just /runs.
"""
...
def create_runs_router(
    ...
    Example:
        ...
        >>> from spine.execution import Dispatcher, create_runs_router
        >>> from spine.execution.executors import LocalExecutor
        >>> 
        >>> app = FastAPI()
        >>> dispatcher = Dispatcher(executor=LocalExecutor())
    ...
    async def list_runs(
        kind: Optional[Literal["task", "pipeline", "workflow", "step"]] = None,
