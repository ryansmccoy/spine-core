...
class TestLogContext:
    ...
    def test_to_dict_excludes_none(self):
        ctx = LogContext(execution_id="abc-123", pipeline=None)
        d = ctx.to_dict()
    ...
    def test_merge_creates_new_context(self):
        ...
        ctx2 = ctx1.merge(pipeline="otc.ingest_week")

        assert ctx1.pipeline is None
        assert ctx2.execution_id == "abc-123"
        assert ctx2.pipeline == "otc.ingest_week"
...
class TestContextManagement:
    ...
    def test_set_context_returns_context(self):
        ctx = set_context(execution_id="test-123", pipeline="test.pipeline")

        assert ctx.execution_id == "test-123"
        assert ctx.pipeline == "test.pipeline"
    ...
    def test_bind_context_merges(self):
        set_context(execution_id="test-123")
        bind_context(pipeline="test.pipeline")
        ctx = get_context()
...
class TestLoggingIntegration:
    ...
    def test_execution_context_flows_through_logs(self):
        ...
        set_context(execution_id="int-test-123", pipeline="test.integration")
        ...
        assert ctx.pipeline == "test.integration"
