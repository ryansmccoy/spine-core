...
class ExecutionHealthChecker:
    ...
    def _check_database(self) -> HealthCheckResult:
        ...
        try:
            ...
            executions = self._ledger.list_executions(limit=1)
    ...
    def _check_stale_executions(self) -> HealthCheckResult:
        ...
        try:
            ...
            critical_stale = self._repo.get_stale_executions(
                older_than_minutes=thresholds.stale_critical_minutes
            )
            
            # Check for warning stale (shorter threshold)
            warning_stale = self._repo.get_stale_executions(
                older_than_minutes=thresholds.stale_warning_minutes
            )
